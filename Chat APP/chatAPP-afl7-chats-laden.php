...
//  https://www.youtube.com/watch?v=9Yz6lCH5Mdc//
//                                             //
//                                             //
/////////////////////////////////////////////////

/* ChatRoom uitlezen - CHAT APP #7 */
...
if (isset($_SESSION['inloggenAPP']) && ($_SESSION['inloggenAPP'] != '')){
	...
	function chat (){
		...
		$chechRoom = mysqli_query($MySQL, 'SELECT `roomId` FROM `Rooms` WHERE `roomId` = "'.$_GET['room'].'"');
		if (mysqli_num_rows($chechRoom) == 1){
			$roomInfo = mysqli_query($MySQL, 'SELECT * FROM `Chats` WHERE `chatRoomId` = "'.$_GET['room'].'"');
	...
	function home (){
		...
		while ($data = mysqli_fetch_assoc($rooms)){
			...
			if ($oke == 1){
				echo '<section>
				<h1><a href="/chat/'.$data['roomId'].'/">'.$data['roomName'].'</a></h1>
				<p>Chat APP</p>
				</section>';
			}
		}
